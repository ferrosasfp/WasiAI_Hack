...
import {LicenseNFTV2 as LicenseNFT} from "./LicenseNFTV2.sol";
import {IAgentRegistry} from "./interfaces/IAgentRegistry.sol";
import {ISplitterFactory} from "./interfaces/ISplitterFactory.sol";

/// @title MarketplaceV3 - AI Model Marketplace with Integrated Revenue Splitters
/// @notice Marketplace for listing AI models with automatic per-model revenue splitters
/// @dev Implements security best practices: Ownable2Step, ReentrancyGuard, Pausable, Timelocks
/// @dev Creates splitter automatically on model publication for x402 inference payments
/// @custom:security-contact security@wasiai.com
contract MarketplaceV3 is Ownable2Step, ReentrancyGuard, Pausable {
    ...
    /// @notice SplitterFactory for creating per-model revenue splitters
    ...
    /// @notice Set SplitterFactory contract for per-model revenue splitters
    /// @param _factory SplitterFactory address
    function setSplitterFactory(address _factory) external onlyOwner {
        if (_factory == address(0)) revert ZeroAddress();
        splitterFactory = ISplitterFactory(_factory);
        emit SplitterFactorySet(_factory);
    }
    ...
    function _listOrUpgradeInternal(
        ...
        // Create per-model splitter for x402 payments
        _createSplitterIfNeeded(id, msg.sender, royaltyBps_);
    }
    
    /// @dev Create splitter for a model if factory is configured
    function _createSplitterIfNeeded(uint256 modelId, address owner, uint256 royaltyBps_) internal {
        if (address(splitterFactory) != address(0)) {
            if (!splitterFactory.splitterExists(modelId)) {
                try splitterFactory.createSplitter(
                    modelId,
                    owner,      // seller (model owner)
                    owner,      // creator (same as owner for first version)
                    royaltyBps_
                ) returns (address splitter) {
                    emit SplitterCreated(modelId, splitter);
                } catch {
                    // Non-critical: splitter creation failed but model succeeds
                    // The splitter can be created manually later via factory
                }
            }
        }
    }
    ...
    /// @notice Get the splitter address for a model
    /// @param modelId Model ID
    /// @return splitter Splitter address (or zero if not exists or factory not set)
    function getModelSplitter(uint256 modelId) external view returns (address splitter) {
        if (address(splitterFactory) == address(0)) return address(0);
        return splitterFactory.getSplitter(modelId);
    }
    
    /// @notice Get model info including splitter address
    /// @param modelId Model ID
    /// @return owner Model owner
    /// @return splitter Splitter address for x402 payments
    /// @return priceInference Inference price
    /// @return royaltyBps Royalty basis points
    function getModelPaymentInfo(uint256 modelId) external view returns (
        address owner,
        address splitter,
        uint256 priceInference,
        uint256 royaltyBps
        ...
        Model storage m = models[modelId];
        owner = m.owner;
        priceInference = m.priceInference;
        royaltyBps = m.royaltyBps;
        
        if (address(splitterFactory) != address(0)) {
            splitter = splitterFactory.getSplitter(modelId);
        }
    }
}
